...
def save_as_text_files(self, data: Dict[str, Any], output_dir: str = "/mnt/HDD2/he110/Linly-Talker/LLM_Chat/itri_museum_docs/itri_comprehensive_data"):        """Save all extracted data as text files"""        os.makedirs(output_dir, exist_ok=True)                print(f"Saving data to {output_dir}/")                # Save metadata        with open(os.path.join(output_dir, 'metadata.txt'), 'w', encoding='utf-8') as f:            f.write("# ITRI Wikipedia Crawler Metadata\n\n")            for key, value in data['metadata'].items():                f.write(f"{key}: {value}\n")                # Save infobox data        if data['infobox_data']:            with open(os.path.join(output_dir, 'infobox.txt'), 'w', encoding='utf-8') as f:                f.write("# ITRI Infobox Data\n\n")                for key, value in data['infobox_data'].items():                    f.write(f"{key}: {value}\n")                # Save sections        for section_name, content in data['sections'].items():            safe_name = re.sub(r'[^\w\s-]', '', section_name).replace(' ', '_')            if isinstance(content, str):                with open(os.path.join(output_dir, f'section_{safe_name}.txt'), 'w', encoding='utf-8') as f:                    f.write(f"# {section_name}\n\n")                    f.write(content)            elif isinstance(content, dict):                with open(os.path.join(output_dir, f'section_{safe_name}.txt'), 'w', encoding='utf-8') as f:                    f.write(f"# {section_name}\n\n")                    for subsection_name, subsection_content in content.items():                        f.write(f"## {subsection_name}\n\n")                        f.write(subsection_content)                        f.write("\n\n")                # Save tables        if data['tables']:            with open(os.path.join(output_dir, 'tables.txt'), 'w', encoding='utf-8') as f:                f.write("# ITRI Wikipedia Tables\n\n")                for table_name, table_data in data['tables'].items():                    f.write(f"## {table_name}\n")                    f.write(f"Rows: {table_data['row_count']}, Columns: {table_data['column_count']}\n\n")                    for row in table_data['rows']:                        f.write("| " + " | ".join(row) + " |\n")                    f.write("\n")                # Save lists        if data['lists']:            with open(os.path.join(output_dir, 'lists.txt'), 'w', encoding='utf-8') as f:                f.write("# ITRI Wikipedia Lists\n\n")                for list_name, list_data in data['lists'].items():                    f.write(f"## {list_name} ({list_data['type']})\n")                    f.write(f"Items: {list_data['item_count']}\n\n")                    for item in list_data['items']:                        f.write(f"- {item}\n")                    f.write("\n")                # Save references        if data['references']:            with open(os.path.join(output_dir, 'references.txt'), 'w', encoding='utf-8') as f:                f.write("# ITRI Wikipedia References\n\n")                for i, ref in enumerate(data['references'], 1):                    f.write(f"{i}. {ref}\n")                # Save external links        if data['external_links']:            with open(os.path.join(output_dir, 'external_links.txt'), 'w', encoding='utf-8') as f:                f.write("# ITRI Wikipedia External Links\n\n")                for i, link in enumerate(data['external_links'], 1):                    f.write(f"{i}. {link}\n")                # Save categories        if data['categories']:            with open(os.path.join(output_dir, 'categories.txt'), 'w', encoding='utf-8') as f:                f.write("# ITRI Wikipedia Categories\n\n")                for category in data['categories']:                    f.write(f"- {category}\n")                # Save raw text        with open(os.path.join(output_dir, 'raw_text.txt'), 'w', encoding='utf-8') as f:            f.write("# ITRI Wikipedia Raw Text\n\n")

f.write(data['raw_text'])                # Save comprehensive JSON        with open(os.path.join(output_dir, 'comprehensive_data.json'), 'w', encoding='utf-8') as f:            json.dump(data, f, ensure_ascii=False, indent=2)                # Create summary file        with open(os.path.join(output_dir, 'summary.txt'), 'w', encoding='utf-8') as f:            f.write("# ITRI Wikipedia Crawler Summary\n\n")            f.write(f"Organization: {data['metadata']['organization_name']}\n")            f.write(f"Crawled at: {data['metadata']['crawled_at']}\n")            f.write(f"Source URL: {data['metadata']['source_url']}\n\n")            f.write(f"Total sections: {data['metadata']['total_sections']}\n")            f.write(f"Total content length: {data['metadata']['total_content_length']} characters\n")            f.write(f"Infobox items: {len(data['infobox_data'])}\n")            f.write(f"Tables: {len(data['tables'])}\n")            f.write(f"Lists: {len(data['lists'])}\n")            f.write(f"References: {len(data['references'])}\n")            f.write(f"External links: {len(data['external_links'])}\n")            f.write(f"Categories: {len(data['categories'])}\n\n")                        f.write("## Section Names:\n")            for section_name in data['sections'].keys():                f.write(f"- {section_name}\n")                print("All data saved successfully!")        print(f"Files created in {output_dir}/:")        print("- metadata.txt")        print("- infobox.txt")        print("- section_*.txt (multiple files)")        print("- tables.txt")        print("- lists.txt")        print("- references.txt")        print("- external_links.txt")        print("- categories.txt")        print("- raw_text.txt")        print("- comprehensive_data.json")        print("- summary.txt")
