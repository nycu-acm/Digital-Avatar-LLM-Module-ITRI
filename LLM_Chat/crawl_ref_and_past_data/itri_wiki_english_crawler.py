...
class ITRIEnglishCrawler:
    def __init__(self):
        self.base_url = "https://en.wikipedia.org/wiki/Industrial_Technology_Research_Institute"
    ...
    def fetch_page(self) -> str:
        """Fetch the English ITRI Wikipedia page"""
        print(f"Fetching page from: {self.base_url}")
        try:
            response = self.session.get(self.base_url, timeout=30)
            response.raise_for_status()
            print("Successfully fetched the English Wikipedia page")
    ...
    def extract_all_content(self, html_content: str) -> Dict[str, Any]:
        """Extract all content from the English Wikipedia page"""
        print("Extracting comprehensive content from English page...")
    ...
    def _extract_infobox(self, soup: BeautifulSoup, data: Dict[str, Any]):
        """Extract data from the Wikipedia infobox"""
        ...
        if infobox:
            ...
            print(f"Extracted {len(infobox_data)} infobox items")
        else:
    ...
    def _extract_references_and_links(self, soup: BeautifulSoup, data: Dict[str, Any]):
        ...
        ext_links_section = soup.find('div', {'id': 'mw-content-text'})
        if ext_links_section:
            external_links = []
            for link in ext_links_section.find_all('a', href=True):
                href = link.get('href', '')
    ...
    def _extract_raw_text(self, content_div: BeautifulSoup, data: Dict[str, Any]):
        """Extract the entire clean text content"""
    ...
    def save_as_text_files(self, data: Dict[str, Any], output_dir: str = "/mnt/HDD2/he110/Linly-Talker/LLM_Chat/itri_museum_docs/itri_english_data"):
        """Save all extracted data as text files"""
        os.makedirs(output_dir, exist_ok=True)
        
        # Save metadata
        with open(os.path.join(output_dir, 'metadata.txt'), 'w', encoding='utf-8') as f:
            f.write("ITRI English Wikipedia Metadata\n")
            f.write("=" * 50 + "\n\n")
        ...
        with open(os.path.join(output_dir, 'infobox.txt'), 'w', encoding='utf-8') as f:
            f.write("ITRI English Wikipedia Infobox\n")
            f.write("=" * 50 + "\n\n")
            for key, value in data['infobox'].items():
                f.write(f"{key}: {value}\n")
        
        # Save sections
        with open(os.path.join(output_dir, 'sections.txt'), 'w', encoding='utf-8') as f:
            f.write("ITRI English Wikipedia Sections\n")
            f.write("=" * 50 + "\n\n")
        ...
        with open(os.path.join(output_dir, 'tables.txt'), 'w', encoding='utf-8') as f:
            f.write("ITRI English Wikipedia Tables\n")
            f.write("=" * 50 + "\n\n")
        ...
        with open(os.path.join(output_dir, 'lists.txt'), 'w', encoding='utf-8') as f:
            f.write("ITRI English Wikipedia Lists\n")
            f.write("=" * 50 + "\n\n")
            for i, list_data in enumerate(data['lists']):
        ...
        with open(os.path.join(output_dir, 'references.txt'), 'w', encoding='utf-8') as f:
            f.write("ITRI English Wikipedia References\n")
            f.write("=" * 50 + "\n\n")
        ...
        with open(os.path.join(output_dir, 'external_links.txt'), 'w', encoding='utf-8') as f:
            f.write("ITRI English Wikipedia External Links\n")
            f.write("=" * 50 + "\n\n")
        ...
        with open(os.path.join(output_dir, 'categories.txt'), 'w', encoding='utf-8') as f:
            f.write("ITRI English Wikipedia Categories\n")
            f.write("=" * 50 + "\n\n")
        ...
        with open(os.path.join(output_dir, 'raw_text.txt'), 'w', encoding='utf-8') as f:
            f.write("ITRI English Wikipedia Raw Text\n")
            f.write("=" * 50 + "\n\n")
        ...
        with open(os.path.join(output_dir, 'summary.txt'), 'w', encoding='utf-8') as f:
            f.write("ITRI English Wikipedia Crawl Summary\n")
            f.write("=" * 50 + "\n\n")
    ...
    def crawl(self, output_dir: str = "/mnt/HDD2/he110/Linly-Talker/LLM_Chat/itri_museum_docs/itri_english_data"):
        """Main crawling method"""
        print("Starting Comprehensive ITRI English Wikipedia Crawler...")
        print("=" * 50)
